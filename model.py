...
def process():
    ...
    for file1,file2 in file_pairs:
        ...
        common_topics=compare_topics(file1, file2, db=db)
        # if len(common_topics)>0:
        #     db.create_relationship(file1, file2, "TOPICS_IN_COMMON_WITH")
        #     print(f"\t*RELATIONSHIP CREATED {file1} & {file2} \n Topics In Common")
        
        print("\n🔹 Named Entities:")
        has_common_name_entities= True if len([i for i in shares_named_entities(file1, file2, db=db) if len(str(i))>3]) > 0 else False
        print("\n🔹 Common Keywords:")
        has_common_key_words= True if len(contains_common_keywords(file1, file2, db=db)) > 0 else False
